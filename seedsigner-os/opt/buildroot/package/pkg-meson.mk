...
# Meson can choke on pkg-config output when sysroot paths contain non-ASCII
# characters. Allow build.sh to inject ASCII aliases for the host and sysroot.
PKG_MESON_HOST_DIR = $(if $(BR2_MESON_HOST_DIR),$(BR2_MESON_HOST_DIR),$(HOST_DIR))
PKG_MESON_STAGING_DIR = $(if $(BR2_MESON_STAGING_DIR),$(BR2_MESON_STAGING_DIR),$(STAGING_DIR))
PKG_MESON_PKGCONF_HOST_BINARY = $(if $(BR2_MESON_PKGCONF_HOST_BINARY),$(BR2_MESON_PKGCONF_HOST_BINARY),$(HOST_DIR)/bin/pkgconf)
...
define PKG_MESON_CROSSCONFIG_SED
        ...
        -e "s%@PKGCONF_HOST_BINARY@%$(PKG_MESON_PKGCONF_HOST_BINARY)%g" \
        -e "s%@HOST_DIR@%$(PKG_MESON_HOST_DIR)%g" \
        -e "s%@STAGING_DIR@%$(PKG_MESON_STAGING_DIR)%g" \
